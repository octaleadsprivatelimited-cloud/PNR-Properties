# Script to copy JB's Serene County images and video once downloads complete
...
Write-Host "Checking and copying JB's Serene County files...`n" -ForegroundColor Cyan

# Image 1
$src1 = Join-Path $downloadsPath "WhatsApp Image 2025-11-20 at 1.14.59 PM.jpeg"
$dest1 = Join-Path $destPath "serene-county-1.jpeg"

if (Test-Path $src1) {
    Copy-Item $src1 -Destination $dest1 -Force
    $size = (Get-Item $src1).Length
    Write-Host "[OK] Copied serene-county-1.jpeg ($([math]::Round($size/1KB, 2)) KB)" -ForegroundColor Green
...
# Image 2
$src2 = Join-Path $downloadsPath "WhatsApp Image 2025-11-20 at 1.15.00 PM.jpeg"
$dest2 = Join-Path $destPath "serene-county-2.jpeg"

if (Test-Path $src2) {
    Copy-Item $src2 -Destination $dest2 -Force
    $size = (Get-Item $src2).Length
    Write-Host "[OK] Copied serene-county-2.jpeg ($([math]::Round($size/1KB, 2)) KB)" -ForegroundColor Green
...
# Video
$srcVideo = Join-Path $downloadsPath "WhatsApp Video 2025-11-20 at 1.18.30 PM.mp4"
$destVideo = Join-Path $destPath "serene-county-video.mp4"

if (Test-Path $srcVideo) {
    Copy-Item $srcVideo -Destination $destVideo -Force
    $size = (Get-Item $srcVideo).Length
    Write-Host "[OK] Copied serene-county-video.mp4 ($([math]::Round($size/1MB, 2)) MB)" -ForegroundColor Green

# Script to copy Nature Valley images and video once downloads complete
...
Write-Host "Checking and copying Nature Valley files...`n" -ForegroundColor Cyan

# Image 1
$src1 = Join-Path $downloadsPath "WhatsApp Image 2025-11-19 at 10.32.51 PM.jpeg"
$dest1 = Join-Path $destPath "nature-valley-1.jpeg"

if (Test-Path $src1) {
    Copy-Item $src1 -Destination $dest1 -Force
    $size = (Get-Item $src1).Length
    Write-Host "[OK] Copied nature-valley-1.jpeg ($([math]::Round($size/1KB, 2)) KB)" -ForegroundColor Green
...
# Image 2
$src2 = Join-Path $downloadsPath "WhatsApp Image 2025-11-19 at 10.32.50 PM.jpeg"
$dest2 = Join-Path $destPath "nature-valley-2.jpeg"

if (Test-Path $src2) {
    Copy-Item $src2 -Destination $dest2 -Force
    $size = (Get-Item $src2).Length
    Write-Host "[OK] Copied nature-valley-2.jpeg ($([math]::Round($size/1KB, 2)) KB)" -ForegroundColor Green
...
# Video
$src3 = Join-Path $downloadsPath "WhatsApp Video 2025-11-19 at 10.40.53 PM.mp4"
$dest3 = Join-Path $destPath "nature-valley-video.mp4"

if (Test-Path $src3) {
    Copy-Item $src3 -Destination $dest3 -Force
    $size = (Get-Item $src3).Length
    Write-Host "[OK] Copied nature-valley-video.mp4 ($([math]::Round($size/1MB, 2)) MB)" -ForegroundColor Green
